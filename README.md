...
**The before/after images look almost identical.** Check that the photo actually shows a smile with teeth visible — if the mouth is closed, there's nothing for either the AI or the free filter to whiten or straighten. Also check whether the small note below the result says "Generated with AI smile simulation" — if it's missing, AI mode likely isn't set up yet (see above), and the free filter's effect is intentionally subtle even with teeth visible.
